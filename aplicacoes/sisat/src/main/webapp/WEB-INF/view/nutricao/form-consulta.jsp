...
<c:if test="${action eq 'cadastrar' }">
	<c:url var="url"
		value="/consulta/realizar-consulta/${consultaNutricional.paciente.pessoa.cpf}"></c:url>
	<c:set var="titulo" value="Nova Consulta "></c:set>
	<c:set var="botao" value="Finalizar Consulta "></c:set>
</c:if>
<c:if test="${action eq 'editar' }">
	<c:url var="url"
		value="/consulta/editar-consulta/${consultaNutricional.id}/paciente/${consultaNutricional.paciente.pessoa.cpf}"></c:url>
		...
		<div class="row">
			...
			<div class="col-sm-9">
				<form:form servletRelativeAction="${url}" method="POST"
					id="consultaNutricional" modelAttribute="consultaNutricional"
